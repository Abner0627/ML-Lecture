...
def load_training_data(path='training_label.txt'):
    ...
    if 'training_label' in path:
        with open(path, 'r', encoding="utf-8") as f:
            lines = f.readlines()
            lines = [line.strip('\n').split(' ') for line in lines]    # 用' '區分字元
...
def load_testing_data(path='testing_data'):
    ...
    with open(path, 'r', encoding="utf-8") as f:
        lines = f.readlines()
        X = ["".join(line.strip('\n').split(",")[1:]).strip() for line in lines[1:]]    # 將段落合併
        X = [sen.split(' ') for sen in X]    #用' '區分字元
    return X
...
def train_word2vec(x):
    ...
    window:    代表input word 與預測word的最大距離，若設為1，代表向左向右各看一個詞，例如「喜歡 麥當當 蘋果派 又 甜 又 香」，
               以「蘋果派」向左向右各看一個詞，就是「麥當當 蘋果派 又」
    iter:      迭代數量，預設為5
    workers:   線程
    '''
    return model
...
if __name__ == "__main__":    
    # 當.py文件被直接運行時，if __name__ == '__main__'之下的代碼塊將被運行；當.py文件以模塊形式被導入時，if __name__ == '__main__'之下的代碼塊不被運行。
...
class Preprocess():
    ...
    def get_w2v_model(self):
        ...
        self.embedding = Word2Vec.load(self.w2v_path).wv
        self.embedding_dim = self.embedding.vector_size
    def add_embedding(self, word):
        # 把 word 加進 embedding，並賦予他一個隨機生成的 representation vector
        # word 只會是 "<PAD>" 或 "<UNK>"
        vector = torch.empty(1, self.embedding_dim)
        torch.nn.init.uniform_(vector)    # 使值服從均勻分佈U(a,b)
    ...
    def make_embedding(self, load=True):
        ...
        for i, word in enumerate(self.embedding.vocab):
            if (i == len(self.embedding.vocab) - 1):
